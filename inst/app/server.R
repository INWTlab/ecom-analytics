...
### Shop Level Analytics ###
  ...
  output$revenueKpi <- renderPrint({
    ...
    infoBox(title = "Total Revenue", revenue,icon = icon("dollar"), color = "black", width = 12)
  })
  ...
  output$customersKpi <- renderPrint({
    ...
    infoBox("Total Customers", customers, icon = icon("users"),color = "black", width = 12)
  })
  ...
  output$numProductsKpi <- renderPrint({
    ...
    infoBox("Total Number of dif. Products", numProducts, icon = icon("cube"),color = "black", width = 12)
  })

  getTopProducts <- reactive({
    if(inputType() != "File" | !is.null(input$file1)){
      ecomData <- getData()
      top <- calcTopProductsShop(ecomData, numProducts = input$numProducts, dateSpan = input$productsSpanVar)
    } else {
      top <- emptyProdTable()
    }
    top
  ...
  getLowProducts <- reactive({
    if(inputType() != "File" | !is.null(input$file1)){
      ecomData <- getData()
      low <- calcLowProductsShop(ecomData, numProducts = input$numProducts, dateSpan = input$productsSpanVar)
    } else {
      low <- emptyProdTable()
    }
    low
  ...
  getTrend <- reactive({
    ecomData <- getData()
    plot <- trendDist(ecomData, dateSpan = input$trendSpanVar, trendVar = input$trendVar)
    plot
  })
...
### Individual Level Analysis ###
  ...
  output$quantileKpiI <- renderPrint({
    ...
    infoBox("Customer's Quantile (by Revenue)", paste0('Top ', quantile, '%'), icon = icon("tachometer"),color = "black", width = 12)
  })
  ...
  getTopProductsI <- reactive({
    if(inputType() != "File" | !is.null(input$file1)){
      ecomData <- getData()
      top <- calcTopProductsI(ecomData, customerID = input$customerId, numProducts = input$numProducts, dateSpan = input$productsSpanVarI)
    } else {
      top <- emptyProdTable()
    }
    top
  ...
  getLowProductsI <- reactive({
    if(inputType() != "File" | !is.null(input$file1)){
      ecomData <- getData()
      low <- calcLowProductsI(ecomData, customerID = input$customerId, numProducts = input$numProducts, dateSpan = input$productsSpanVarI)
    } else {
      low <- emptyProdTable()
    }
    low
 ...
 ### Beobachten bestimmter Aktionen ###
