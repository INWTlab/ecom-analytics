...
trendDist <- function(ecomData, dateSpan, trendVar) {
  trendData <- ecomData %>% filter((DateComplete > dateSpan[1]) & (DateComplete < dateSpan[2]))  %>% group_by(DateComplete) %>% summarise(summation = sum(get(trendVar)))
  ggplot(trendData,   aes(x=DateComplete, y=summation)) + geom_line(color = "#b3d0ec") + labs(x="Period", y=trendVar)
  }
...
trendDistI <- function(ecomData, customerID, dateSpan, trendVarI) {
  ...
  trendData <- ecomData %>% filter((DateComplete > dateSpan[1]) & (DateComplete < dateSpan[2]))  %>% group_by(DateComplete) %>% summarise(summation = sum(get(trendVarI)))
  ggplot(trendData,   aes(x=DateComplete, y=summation)) + geom_line(color = "#b3d0ec") + labs(x="Period", y=trendVarI) + geom_point()
}
